...
const Message: React.FC<MessageProps> = ({ message, handleAction, retryLastMessage }) => {
  ...
  const renderContent = () => {
      ...
      return (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm sm:text-base">{message.content as string}</p>
          </div>
          <button
            onClick={retryLastMessage}
            className="w-full text-sm font-bold py-2 px-4 rounded-lg transition-colors duration-300 bg-purple-600 text-white hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500"
          >
            إعادة المحاولة
          </button>
        </div>
    ...
    return <p className="text-sm sm:text-base whitespace-pre-wrap">{message.content as string}</p>;
  };
    ...
    return (
      <div className="flex items-end justify-start">
        <div className="flex flex-col space-y-2 text-base max-w-xs mx-2 order-2 items-start">
          ...
          {/* Only render the text bubble if there is text content */}
  ...
  if (message.type === 'loading') {
    return null; // TypingIndicator will handle this
  }
  ...
  return (
    <div className="flex items-end">
      ...
      <div className="flex flex-col space-y-1 text-base max-w-xs mx-2 order-2 items-start">
        <div>
          <div className={`px-4 py-2 rounded-lg inline-block rounded-bl-none ${
